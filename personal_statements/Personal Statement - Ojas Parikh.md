...
**Final Design:** [`4cf954b`](https://github.com/pharquissandas/RISC-V-T19/commit/4cf954b8ce1225789a7f21062ec9ee397945cd97)
...
* **Verification and Debugging:** I heavily utilised the .vcd waveform files generated by Verilator in order to debug my design. With the waveforms, I traced through the what each stage was doing cycle by cycle to identify issues with my implementation. Currently, my design passes 49/49 tests in our [verify.cpp](https://github.com/pharquissandas/RISC-V-T19/blob/superscalar/tb/tests/verify.cpp) file, however a somewhat interesting thing was that Test SLTIU passes only if you set the number of cycles to certain values, as the program as it is written is an infinite loop so the time at which the testbench ends effects the output value. In order to test that my design was indeed correct for SLTIU I both changed the cycle number to 10001 and also changed the source register for SLTIU to rs1 so that the loop is exited. In botht these situations the test case passes. I did not have time to verify the design with the VBuddy tests.
